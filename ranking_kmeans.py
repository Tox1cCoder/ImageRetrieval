...
def main():
    ...
    feat = all_feats[-1]
    cluster_assignment = kmeans.predict(np.expand_dims(feat, axis=0))[0]
    cluster_indices = np.where(kmeans.labels_ == cluster_assignment)[0]

    distances = cosine_distances(np.expand_dims(feat, axis=0), all_feats[cluster_indices])
    ...
    with open(
            evaluate_root + '/' + path_crop + '/' + args.feature_extractor + '/' + path_file[:-10] + '.txt',
            "w") as file:
        file.write("\n".join(rank_list))
